...
func PostToAgent(metrics []*model.MetricValue) {
	...
	if err != nil {
		log.Printf("Error for PostToAgent json Marshal: %v", err)
		return
	}
	contentReader := bytes.NewReader(contentJson)
	req, err := http.NewRequest("POST", Config().Transfer.Addr, contentReader)
	if err != nil {
		log.Printf("Error for PostToAgent in NewRequest: %v", err)
		return
	}
	...
	if err != nil {
		log.Printf("Error for PostToAgent in http client Do: %v", err)
		return
	}
